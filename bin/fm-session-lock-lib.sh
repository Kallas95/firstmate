...
# Print the id of the Claude session that delivered hook payload $1 only when
# the event identity is internally consistent and its exported session pid is a
# live Claude process. The payload describes this event; the matching exported
# id and live pid keep an inherited environment from speaking for it.
fm_claude_hook_event_session() {  # <payload>
  local payload=${1-} payload_session
  case "${CLAUDE_PID:-}" in
    ''|*[!0-9]*) return 1 ;;
  esac
...
fm_session_lock_owned_by_claude_hook() {  # <state-dir> <payload>
  local state=$1 payload=${2-} lock_pid
  fm_claude_hook_event_session "$payload" >/dev/null || return 1
...
fm_session_lock_publish_identity() {  # <state-dir> <lock-pid> [Claude hook payload]
...
fm_session_lock_owned_by_claude_hook_binding() {  # <state-dir> <payload>
  ...
  payload_session=$(fm_claude_hook_event_session "$payload") || return 1
  fm_session_lock_binding_names_session "$state" "$payload_session"
}

# True when the session behind the current run owns state dir $1's fleet lock.
# An ordinary caller uses harness ancestry or its ancestry-corroborated durable
# identity. A native Claude session-open descendant additionally carries the
# validated event payload, so the exact delivered pid or the lock's matching
# durable binding can authorize work after a shared pool is reparented.
#
# This is the ONE owner of "may this run mutate this home", so lock acquisition,
# context re-emission, and the deferred startup sweeps cannot drift apart. A
...
fm_session_lock_owned_by_current_session() {  # <state-dir>
  ...
  fm_session_lock_owned_by_session_identity "$state"
}
...
#
# All three members are load-bearing. Ancestry remains available when it reaches
# the exported session pid, because a legitimate claude-launched-by-claude
# wrapper chain can record an outer process while CLAUDE_PID names the inner
# session. When a valid event's ancestry cannot reach that pid, generic ancestry
# is a shared-pool fact rather than a session fact and is not accepted. The exact
# delivered pid handles a reparented first acquisition, while the durable binding
# handles an older or outer-process lock owned by that same delivered session.
# None is a fallback for the others: a caller needing one still calls it directly.
#
...
fm_session_lock_owned_by_this_claude_session() {  # <state-dir> <payload>
  ...
  if [ -n "$event_session" ] && ! fm_harness_session_is_ours; then
    ancestry_allowed=0
  fi
  if [ "$ancestry_allowed" -eq 1 ] && fm_session_lock_owned_by_self "$state"; then
