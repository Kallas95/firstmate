...
`FM_CLAUDE_TURNEND_STALL_BUDGET` (default 8) therefore counts the blocked stops themselves, unkeyed, and the stop that passes it opens the same one attended alarm without requiring the verified failure episode below, because that evidence can only be produced by an auto-arm that runs.
...
It also covers the consecutive-block bound over a frozen ledger, asserting that the epoch-keyed count stays frozen so the case cannot pass vacuously, that a stop the guard lets through ends the run, and that away mode never opens the alarm.
