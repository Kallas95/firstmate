...
`FM_CLAUDE_TURNEND_STALL_BUDGET` (default 7) therefore counts the blocked stops themselves, unkeyed, and the stop that passes it opens the same one attended alarm without requiring the verified failure episode below, because that evidence can only be produced by an auto-arm that runs. The default opens on the eighth Stop attempt, after seven actual blocks and before Claude's hard override after eight consecutive blocks.
