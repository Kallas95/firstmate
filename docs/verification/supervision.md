...
When both a corroborated caller identity and a valid binding exist, the binding is authoritative over generic ancestry: only the session it names is admitted, even when another session runs through the same shared pool ancestry.
A publication failure or binding mismatch leaves the verified lock owner and valid binding unchanged, refuses mutation, and directs the caller to operate read-only; an already-valid matching binding is preserved.
...
`tests/fm-session-lock-identity.test.sh` pins the resulting logic portably with deterministic process tables and a real nested shared-pool process tree, including two session identities routed through the same live worker and host with only the bound session admitted, required-publication failure injection at first acquisition and existing-lock backfill, ancestry-only success without a usable identity, serialized legacy-binding backfill, valid-binding preservation, and concurrent-takeover refusal.
