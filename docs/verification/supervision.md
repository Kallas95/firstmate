...
`tests/fm-session-lock-identity.test.sh` pins the resulting logic portably with deterministic process tables and live process owners, including two sessions sharing one pool ancestry with only the bound session admitted, required-publication failure injection at first acquisition and existing-lock backfill, ancestry-only success without a usable identity, serialized legacy-binding backfill, valid-binding preservation, and concurrent-takeover refusal.
