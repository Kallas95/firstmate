...
A working Pi, pending input, missing or contradictory identity, truncated capture, incomplete separator pair, over-tall candidate, or shell remains unknown or pending.
Identity stays a lazy second read, consulted only when a recognized Pi structure could change the verdict.
