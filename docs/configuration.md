...
FM_AUTOARM_TRACE=       # opt-in diagnostic; any non-empty value names on stderr the gate that ends a Claude Stop auto-arm run inert before it claims the cycle, changes no decision, and is unset in normal operation
