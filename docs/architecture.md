...
A crew whose task carries a still-open captain decision (a `needs-decision`/`blocked` the keyed decision fold has not closed) is absorbed the same way: its inactivity is explained by that open decision, not a wedge, so it re-surfaces on the longer pause cadence instead of wedge-escalating every `FM_STALE_ESCALATE_SECS`.
...
A crew parked on a still-open captain decision trades it the same way, so an abandoned decision cannot rot invisibly either, while the normal wedge detector resumes the moment the decision closes.
