...
That cadence applies whether its agent is still alive on an otherwise healthy idle pane or has already exited, so a legitimate long external wait is not re-escalated every `FM_STALE_ESCALATE_SECS`, while the pane returns to the normal cadence as soon as its last status line is no longer a declared pause or `bin/fm-crew-state.sh` reports it working again.
A durable `captain-held` endpoint joins that same cadence only when the backend confidently reports its agent dead, so the normal-mode watcher still surfaces one stale wake at that non-pause gate when liveness is live or inconclusive, and an ordinary crew whose authoritative state has fallen back to stopped or unknown likewise recovers the paused classification only with a confidently dead agent.
A pane that dies while its declared pause still stands re-surfaces on that bounded recheck instead of staying hidden behind the pause, and the secondmate idle-endpoint exemption is unchanged.
