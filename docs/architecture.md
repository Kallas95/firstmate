...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes with no still-open captain decision that persist past `FM_STALE_ESCALATE_SECS`, declared external waits and decision-parked crews that remain idle past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A provably-working crew whose task carries a still-open captain decision (a `needs-decision`/`blocked` the keyed decision fold has not closed) trades the wedge timer for that same longer pause cadence: its inactivity is explained by that open decision, not a wedge, so it re-surfaces once per window instead of wedge-escalating every `FM_STALE_ESCALATE_SECS`.
The provably-working condition is required, not incidental: a crew that has stopped or died with a decision still open shows no working evidence, so it keeps surfacing immediately under the ordinary stale triage above and an open decision can never swallow a frozen pane.
The criterion is the open decision itself (the same `status_open_decisions` fold the drain's OPEN DECISIONS section uses), not mere inactivity, so a genuinely wedged crew and a decision-waiting crew stay distinguishable.
When the decision closes (a `resolved` or `captain-held` line lands), the normal wedge cadence resumes immediately, so a crew that stays mute after the answer re-escalates exactly as before.
The `needs-decision` itself already surfaced through the signal path (a captain-relevant verb is always actionable), so absorbing the later stale re-surfaces loses nothing.
...
A provably-working crew parked on a still-open captain decision trades it the same way, so an abandoned decision cannot rot invisibly either, while the normal wedge detector resumes the moment the decision closes.
