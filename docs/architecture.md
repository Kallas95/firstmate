...
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) extends this for walk-away supervision: the `/afk` skill starts it through the tracked foreground helper `bin/fm-afk-start.sh`, after which, for as long as that daemon is alive, the watcher reverts to daemon-managed one-shot mode and the daemon self-handles routine wakes in bash.
Liveness, not the `state/.afk` flag, is what hands the watcher over: a flagged home whose daemon died keeps its ordinary triage, per [watcher-continuity.md](watcher-continuity.md#away-mode-stand-down).
