...
`tests/fm-session-lock-identity.test.sh` owns the proofs themselves, including first acquisition and repeat admission through a reparented shared pool plus the foreign-session counter-proof, with each case asserting the inert divergence first so it cannot pass vacuously.
`tests/fm-startup-network.test.sh` proves the same delivered identity authorizes the owning session's deferred mutating sweeps without ancestry and refuses a different delivered session.
