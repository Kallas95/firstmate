...
#
# Every case drives the library behind a deterministic fake ps, so the same
...
test_e2e_first_acquire_from_reparented_pool_records_the_session() {
  local dir fakebin out payload session_pid spare_pid host_pid
  dir=$(new_home e2e-reparented-acquire)
  fakebin=$(fm_fakebin "$dir/bin")
  spawn_live_pid; session_pid=$LIVE_PID
  ...
  out=$(FM_SESSION_LOCK_HOOK_PAYLOAD="$payload" \
    lock_sh "$dir" "$fakebin" "$SESSION" "$session_pid") \
    || fail "the first acquisition from a reparented pool failed: $out"
  [ "$(tr -d '[:space:]' < "$dir/state/.lock")" = "$session_pid" ] \
    || fail "the reparented acquisition recorded the shared pool instead of the session pid"
  grep -qx "pid=$session_pid" "$dir/state/.lock.session" \
    || fail "the reparented acquisition did not bind the session pid"
  grep -qx "session=$SESSION" "$dir/state/.lock.session" \
    || fail "the reparented acquisition did not bind the delivered session identity"

  out=$(FM_SESSION_LOCK_HOOK_PAYLOAD="$payload" \
    lock_sh "$dir" "$fakebin" "$SESSION" "$session_pid") \
    || fail "the owning session was not readmitted through the same pool: $out"
  case "$out" in
    *"lock acquired: harness pid $session_pid"*) ;;
    *) fail "the reparented owner did not retain its session pid: $out" ;;
  esac

  reap_live_pids
  pass "session-lock identity e2e: a first session-open through a reparented pool records and readmits the delivered session"
}
...
test_reparented_event_does_not_trust_a_shared_pool_ancestor() {
  local dir fakebin owner_payload foreign_payload
  dir=$(new_home shared-pool-ancestor)
  fakebin=$(fm_fakebin "$dir/bin")
  write_reparented_pool_ps "$fakebin"
  printf '27305\n' > "$dir/state/.lock"
  bind_identity "$dir/state" 27305 "$SESSION"
  owner_payload=$(stop_payload "$SESSION")
  foreign_payload=$(stop_payload "$OTHER_SESSION")

  # Both events run below the same pool host, so a bare ancestry test accepts
  # either. The delivered identity must narrow that shared ancestry: the bound
  # owner is readmitted and the foreign session is refused.
  FM_SESSION_LOCK_HOOK_PAYLOAD="$owner_payload" \
    lib_eval "$fakebin" "fm_session_lock_owned_by_current_session '$dir/state'" "$SESSION" 89187 \
    || fail "the session bound to a shared pool lock was not readmitted"
  if FM_SESSION_LOCK_HOOK_PAYLOAD="$foreign_payload" \
    lib_eval "$fakebin" "fm_session_lock_owned_by_current_session '$dir/state'" "$OTHER_SESSION" 4242; then
    fail "a foreign session claimed the shared pool ancestor recorded by another session"
  fi
  pass "session-lock identity: a delivered event narrows shared-pool ancestry to its bound session"
}
